...
output_dir <- "~/Desktop/Dimitriou_2025/brms_output"
if (!dir.exists(output_dir)) dir.create(output_dir)

model_data <- read.csv("~/Desktop/Dimitriou_2025/data/model_data.csv")
  ...
  model <- brm(
    formula = as.formula(
      paste(response_var, "~",
            ...
            "rec_month_100_scaled * dist_to_trail_decay +",
...
for (species_name in names(species_models)) {
  ...
  for (param in params) {
    ...
    summary_df <- bind_rows(summary_df, tibble(
      ...
      ESS_bulk = round(ess_vals[param], 0)
    ))
  }
...
# Save random effect standard deviation
# Initialize empty dataframe
re_sd_df <- data.frame()
...
var_labels <- c(
  ...
  "b_rec_month_100_scaled:dist_to_trail_decay" = "Recreation × Distance to Trail"
)
...
for (species_name in names(species_name_map)) {
  ...
  model_path <- paste0("~/Desktop/Dimitriou_2025/brms_output/", species_name, "_model.rds")
  if (!file.exists(model_path)) {
...
trace_dir <- "~/Desktop/Dimitriou_2025/brms_output/diagnostics/trace_plots"
pp_dir <- "~/Desktop/Dimitriou_2025/brms_output/diagnostics/pp_checks"
...
# Save combined plots
ggsave("~/Desktop/Dimitriou_2025/brms_output/diagnostics/Combined_PP_Checks.pdf", pp_combined, width = 12, height = 9)
ggsave("~/Desktop/Dimitriou_2025/brms_output/diagnostics/Combined_Trace_Plots.pdf", trace_combined, width = 10, height = 15)
...
joffre_richness <- plot_species_accumulation_q(
  "~/Desktop/Dimitriou_2025/data/JOFF_30min_Independent_daily_observations.csv",
  "Joffre Lakes", q = 0, y_label = "Species Richness", y_limit = c(0, 15)
)

garibaldi_richness <- plot_species_accumulation_q(
  "~/Desktop/Dimitriou_2025/data/GARI_30min_Independent_daily_observations.csv",
...
# Save plots
ggsave(
  filename = file.path("~/Desktop/Dimitriou_2025/diversity_output", "species_accumulation_4x3_grid.png"),
...
joffre <- process_park_data_periods("~/Desktop/Dimitriou_2025/data/JOFF_30min_Independent_daily_observations.csv", "Joffre Lakes")
garibaldi <- process_park_data_periods("~/Desktop/Dimitriou_2025/data/GARI_30min_Independent_daily_observations.csv", "Garibaldi")
...
# Save estimates
dir.create("~/Desktop/Dimitriou_2025/diversity_output", recursive = TRUE, showWarnings = FALSE)
saveRDS(richness_estimates, "~/Desktop/Dimitriou_2025/diversity_output/richness_estimates.rds")
saveRDS(shannon_estimates, "~/Desktop/Dimitriou_2025/diversity_output/shannon_estimates.rds")
saveRDS(evenness_estimates, "~/Desktop/Dimitriou_2025/diversity_output/evenness_estimates.rds")
...
# Save Results
write.csv(results_df, "~/Desktop/Dimitriou_2025/diversity_output/diversity_bootstrap_BY_results.csv", row.names = FALSE)
